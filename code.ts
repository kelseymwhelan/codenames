...
figma.ui.onmessage = async (msg) => {
  ...
  if (msg.type === 'create-game') {
    ...
    if (existingGameBoard !== null) { existingGameBoard.remove() }
    if (existingReadMe !== null) { existingReadMe.remove() }
    ...
    for (let i = 0; i < numRedCards; i++) {cardColors.push('Red')}
    for (let i=0; i < numBlueCards; i++) {cardColors.push('Blue')}
    for (let i=0; i < 7; i++) {cardColors.push('Tan')}
